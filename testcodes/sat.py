...
class heap(task):
    ...
    def swap(self,a,b):
        c=self.listh[a]
        self.listh[a]=self.listh[b]
        self.listh[b]=c

    def down_heapify(self,i):
        if(2*i+1 == len(self.listh)-1):
            if(self.listh[i].deadline>self.listh[2*i+1].deadline):
                self.swap(i,2*i+1)
        elif (2*i+1 < len(self.listh)-1):
    ...
    def __init__(self,deadlines):
        ...
        for i in deadlines:
            test=task(j,i)
            self.listh.append(test)
            j=j+1
        self.heapify()
            
        
    def insert(self,taskId, taskDeadline):
        taskobj = task(taskId, taskDeadline)
        self.listh.append(taskobj)
        self.up_heapify(len(self.listh)-1)

    def up_heapify(self,a):
        if(a > 0 and (self.listh[(a-1)//2].deadline > self.listh[a].deadline)):
            self.swap((a-1)//2,a)
            self.up_heapify((a-1)//2)
    
    def minimum(self,a,b,c):
        if(self.listh[a].deadline < self.listh[b].deadline):
            mini=a
        else:
            mini=b
            
        if(self.listh[c].deadline<self.listh[mini].deadline):
            mini=c
        return mini
         
    def extract_min(self):
        rem=self.listh[0]
        self.swap(0,len(self.listh)-1)
        self.listh.remove(rem)
        self.down_heapify(0)
        return rem
...
##                print extractedTasks[i].taskid , extractedTasks[i].deadline 
    ...
    for j in range(len(deadlinesHeap.listh)):
